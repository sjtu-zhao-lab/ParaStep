"""
python get_fvd_psnr_ref_file_target_dir.py --reference_path outputs/cogvideox/withbaseline/baseline.mp4 --input_folder outputs/cogvideox/withbaseline2
"""
...
def calculate_psnr(videos1, videos2, data_range=1.0):
    psnr = PeakSignalNoiseRatio(data_range=data_range)
    return psnr(videos1, videos2).item()
...
if __name__ == "__main__":
    ...
    for video_file in video_files:
        ...
        # 计算PSNR (这里使用最短视频长度避免形状不一致)
        min_frames = min(reference_tensor.shape[1], input_tensor.shape[1])
        psnr_score = calculate_psnr(input_tensor[:, :min_frames], reference_tensor[:, :min_frames])

        print(f"Video: {video_file.name} | FVD: {fvd_score:.4f} | PSNR: {psnr_score:.4f} dB")
