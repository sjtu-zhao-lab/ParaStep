"""
CUDA_VISIBLE_DEVICES=0 python tools/common_metrics_on_video_quality/calculate_all.py \
    --gt_video_dir outputs_eval/svd/baseline \
    --generated_video_dir outputs_eval/svd/async2
"""
...
def main(args):
    ...
    vbench = True if args.vbench == "True" else False
    ...
    for idx, video_id in enumerate(tqdm.tqdm(range(total_len))):
        ...
        # if calculate_fvd_flag:
        #     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        #     result = calculate_fvd(gt_videos_tensor, generated_videos_tensor, device=device)
        #     result = result["value"]
        #     result = sum(result) / len(result)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vbench", type=str, default="False")
