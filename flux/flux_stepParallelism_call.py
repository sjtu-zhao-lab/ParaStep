...
def step_parallelism_call(
    ...
    print("step parallelism call for flux")
    height = height or self.default_sample_size * self.vae_scale_factor
    width = width or self.default_sample_size * self.vae_scale_factor
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if (self.inference_step_num <= self.warm_up) or (self.step_size==1):
                noise_pred = self.transformer(
                    hidden_states=latents,
                    timestep=timestep / 1000,
                    guidance=guidance,
            ...
            elif self.inference_step_num == num_inference_steps:
            ...
            if (self.inference_step_num > self.warm_up): 
                if (self.round == self.step_size - 1) or ((self.inference_step_num >= num_inference_steps)):
